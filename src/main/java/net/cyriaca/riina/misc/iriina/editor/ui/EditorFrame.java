...
import net.cyriaca.riina.misc.iriina.editor.data.LoadTarget;
import net.cyriaca.riina.misc.iriina.editor.data.ProjectMeta;
import net.cyriaca.riina.misc.iriina.editor.data.ReversibleOperation;
...
import net.cyriaca.riina.misc.iriina.editor.editor.data.ManipTarget;
import net.cyriaca.riina.misc.iriina.editor.ui.editor.*;
...
public class EditorFrame extends JFrame implements IViewFrame, WindowListener, LocaleChangeListener, ActionListener, LineListener {
    ...
    private void reloadProject() { // use to revert to last saved version
    ...
    public void move(float deltaTime) {
        switch (manipTarget.getType()) {
            ...
            case TIMING_EVENT_ROOT_TIME_DRAG:
                manipTarget.getTarget().getTimingProperty().setRootTime(manipTarget.getTarget().getTimingProperty().getRootTime() + deltaTime);
                break;
        }
    }
    ...
    public float getPlayHeadPercent() {
        return getPlayHeadPos() / getClipLength();
    }

    public void setPlayHeadPercent(float percent) {
        setPlayHeadPos(percent * getClipLength());
    }

    public float getPlayHeadPos() {
        return ((float) clip.getMicrosecondPosition()) / 1000000.0f;
    }
    ...
    public void setSelectionDown(SelectionMode mode, MapEvent.Type type, float t1, float t2, boolean last) {
        ...
        //System.out.print(l.size() + " events");
        for (MapEvent e2 : l) {
            selection.add(e2);
    ...
    private float getClipLength() {
        return ((float) clip.getMicrosecondLength()) / 1000000.0f;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == timer) {
            ...
            eventModContainerPanel.updateEventTargets();
            //eventDisplayPanel.paintImmediately(eventDisplayPanel.getBounds());
        }
